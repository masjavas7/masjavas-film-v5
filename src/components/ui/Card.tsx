...
export const Card: React.FC<CardProps> = ({ children, className = "", ...props }) => {
  return (
    <div
      className={cn("rounded-[28px] border border-white/10 bg-white/[0.065] p-5 shadow-2xl shadow-black/20 backdrop-blur-xl", className)}
      {...props}
    >
      {children}
    </div>
...
interface PillProps {
  children: React.ReactNode;
  tone?: "default" | "soft" | "green" | "amber" | "purple" | "blue";
}
...
export const Pill: React.FC<PillProps> = ({ children, tone = "default" }) => {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-medium", map[tone])}>
      {children}
    </span>
  );
};
